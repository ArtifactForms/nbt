...
public class UnitIntArrayTag {

    private static Tag createTestTag() {
	return new IntArrayTag("IntArrayTag", new int[0]);
    }
	...
	int[] newValue = new int[] { 1, 2, 3, 4, 5 };
	IntArrayTag tag = (IntArrayTag) createTestTag();
	...
	tag.accept(visitor);
	Assert.assertEquals(1, visitor.getVisitedInArrayTagCount());
