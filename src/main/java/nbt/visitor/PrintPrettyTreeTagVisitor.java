...
public class PrintPrettyTreeTagVisitor implements TagVisitor {
    ...
    public void visit(ByteArrayTag tag) {
        append(
                formatTag(
                        "TAG_ByteArray", tag, tag.getValue().length + " bytes"
                )
        );
    }
    ...
    public void visit(CompoundTag tag) {
        append(
                formatTag(
                        "TAG_Compound", tag,
                        tag.getTagCountExceptEndTags() + " entries"
                )
        );
        append(getDepthTabs() + "{\n");
        depth++;
    ...
    public void visit(ListTag tag) {
        append(
                formatTag(
                        "TAG_List:" + tag.getContentType(), tag,
                        tag.getTagCountExceptEndTags() + " entries"
                )
        );
        append(getDepthTabs() + "{\n");
    ...
    public void visit(LongArrayTag tag) {
        append(
                formatTag(
                        "TAG_LongArray", tag, tag.getValue().length + " longs"
                )
        );
    }
