...
public class Schematic {
	...
	public static final int MAX_BLOCK_COUNT = Short.MAX_VALUE * Short.MAX_VALUE * Short.MAX_VALUE;
	...
	private static Schematic createSchematic(short width, short height, short length) {
	...
	private static void validateMaxBlockCount(int width, int height, int length) {
		int blockCount = width * height * length;

		if (blockCount > MAX_BLOCK_COUNT)
			throw new SchematicException("Block count ist greater than MAX_BLOCK_COUNT: " + MAX_BLOCK_COUNT);
	}

	private static void validateSchematicSize(int width, int height, int length) {
		if (width > Short.MAX_VALUE)
			throw new SchematicException("Width is greater than Short.MAX_VALUE.");

		if (height > Short.MAX_VALUE)
			throw new SchematicException("Height is greater than Short.MAX_VALUE.");

		if (length > Short.MAX_VALUE)
			throw new SchematicException("Length is greater than Short.MAX_VALUE.");
	}
	...
	private static void validate(CompoundTag root) {
		...
		if (result.isInvalid()) {
			throw new SchematicException("Invalid schematic: " + result.getCausesAsString());
		}
	}
	...
	private void validateBlockCoordinateZ(int z) {
		if (z >= getLength() || z < 0)
			throw new IndexOutOfBoundsException("The z-coordinate is out of bounds.");
	}
